...
class Event extends AppBase {
    ...
    public function index() {
        $data['page'] = $this->base_model->get_join_item('result', 'posts.*, first_name, last_name', 'posts.id DESC', 'posts', 'users', 'posts.user_id=users.id', 'left', array('post_type' => 'event'));
        $this->admindisplay('manage/information/event/event', $data);
    }
    ...
    public function add() {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if ($this->form_validation->run() === FALSE) {
                $this->admindisplay('manage/information/event/event_add', $data);
            } else {
                ...
                redirect('manage/information/event');
            }
        }
    }
    ...
    public function edit($id = "") {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            $data['page'] = $this->base_model->get_item('row', 'posts', '*', array('post_type' => 'event', 'id' => $id));
            if (!$data['page']) {
                show_404();
            }
            ...
            if ($this->input->post('act', TRUE)) {
                ...
                if ($this->form_validation->run() === FALSE) {
                    $this->admindisplay('manage/information/event/event_edit_f', $data);
                } else {
                    $params = array(
                        'title' => $this->input->post('title', TRUE),
                        'description' => $this->input->post('ck_description_field', TRUE),
                    ...
                    redirect('manage/information/event/edit/' . $id);
                }
            } else {
                $this->admindisplay('manage/information/event/event_edit', $data);
            }
        }
    }
