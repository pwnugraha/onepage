...
class AppBaseSite extends CI_Controller {
    ...
    public function siteview($child_view = "", $data = array()) {
        ...
        $data['article'] = $this->base_model->get_item('row', 'posts', 'description, rel_url', array('post_type' => 'article'), 'posts.id DESC', 1);
        $data['pages'] = $this->base_model->get_item('result', 'posts', 'title, rel_url', array('post_type' => 'page'));
